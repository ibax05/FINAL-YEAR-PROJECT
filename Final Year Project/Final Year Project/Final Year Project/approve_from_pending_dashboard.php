...
if ($row ) {
    ...
    try {
        ...
        $_SESSION['approved_from_dashboard'] = "Permohonan pemohon berjaya di luluskan";
        header("Location:pending_from_dashboard.php");
        
    } catch (Exception $e) {
        $_SESSION['error_email_from_dashboard_approved'] = "Permohonan pemohon tidak berjaya di email kan. Mailer Error: {$mail->ErrorInfo}";
        header("Location:pending_from_dashboard.php");

    }
} else {
    $_SESSION['error_user_from_dashboard_approved'] = "No user found.";
    header("Location:pending_from_dashboard.php");
}
