...
if ($row) {
    ...
    try {
        ...
        $_SESSION['rejected'] = "Permohonan pemohon berjaya di ditolak";
        header("Location:status_permohon.php");
    } catch (Exception $e) {
        $_SESSION['error_email'] = "Permohonan pemohon tidak berjaya di email kan. Mailer Error: {$mail->ErrorInfo}";
        header("Location:status_permohon.php");
    }
} else {
    $_SESSION['error_user'] = "No user found.";
    header("Location:status_permohon.php");
}
